...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(String param : params){
			cnt++;
			if(param.equals("")){
				response.sendRedirect(request.getContextPath()+"/user/login.do?errMsg"+cnt+"=NotNull");
		...
		try{
			...
			response.sendRedirect(request.getContextPath()+"/user/login.do?errMsg1=NumberFormatException");
			return;
		}catch (Exception e) {
